...
function showTitlePane(state,txt,iLeft,iTop) 
{
		...
		if (loadingPane == null) {
			...
			el.style.cssText="background-color:#FFFFFF; padding:15px; display:none;font-family:宋体;font-size:14px;font-weight:bold;line-height:140%; border:5px solid #D8EFFF;position:absolute; right:1px; top:1px; bottom:1px; left:1px;width:18%; height:10%; z-index:10000;filter:alpha(opacity=90);";
	...
	if(imgs != null)
	{
		for(var i=0;i<imgs.length;i++)
		{
			...
			if(typeof mapname == "string" && mapname.toLowerCase() == ("#" + mapObj.name).toLowerCase())
			{
				...
				if(getWidthNumber(imgobj.parentElement.offsetLeft)>=10 && navigator.userAgent.toLowerCase().indexOf("msie 8")!=-1){ imgleft=imgobj.parentElement.offsetLeft-20;imgtop-=20;}
				imgleft = imgleft + rangeDeltaX;
				imgtop = imgtop + rangeDeltaY;
 var articleRe = /nw.(\S+)\.htm/i;
 if(typeof areaObj.href == "string")
 {
 	var r = areaObj.href.match(articleRe);
	if(r)
	{
		var titleObj = $("mp_nw." + r[1]);
