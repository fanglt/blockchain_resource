if   (top!= self){
	//不允许iframe
	if(!/paperpost$/i.test(location.href)){
		top.location=self.location;
	}
}
//配合天润统计
updateMeta(location.href);
function updateMeta(webLastUrl){
	var x=webLastUrl.indexOf("nw.D110000renmrb_");
	var y=webLastUrl.indexOf(".htm");
	if(x>=0){
		var publishdate=webLastUrl.substring(x+17,x+17+8);
		publishdate=publishdate.substring(0,4)+"-"+publishdate.substring(4,6)+"-"+publishdate.substring(6,8);
		var contentid="rmrbdzb_"+webLastUrl.substring(x+17,y);
		 showMeta(publishdate,contentid);
	}
}
function showMeta(publishdate,contentid) {
 	var metas = document.getElementsByTagName("meta");
 	for(var i=0;i<metas.length;i++){
 		if("contentid"==metas[i].name ){
 			metas[i].content=contentid;
 		}else if("publishdate"==metas[i].name ){
 			metas[i].content=publishdate;
 		}
 	}
 }
...
function openwin()
{
	if(location.href.indexOf('people')!=-1)	window.open("http://paper.people.com.cn/rmrb/notice_content.htm","","width=980,height=495")
} 
...
function loadpopup(){ 
	if (get_cookie('popped')==''){ 
		...
		//document.cookie="popped=yes;expires=" + expires.toGMTString();
	} 
} 
//日期控件只显示最近几年的
var selx=$S("SY");
...
selx[0].onchange= changeMPCldrmrb
for(var i=0;i<startYear-2006;i++){
	deleteOption(selx[0],0);//2006
}
for(var i=0;i<2050-nowYear;i++){
	deleteOption(selx[0],nowYear-startYear+1);//2006
}	


function deleteOption(sel,indx){
       sel.removeChild(sel.options[indx]);
}
...
function $S() {    
  ...
  for (var i = 0; i < arguments.length; i++) { // 可以传入多个参数   
    ...
    if (typeof element == 'string') // 如果参数的类型是字符串      
    	element = getElementsByName('select',element);       
...
function drawCld(SY,SM) {
   ...
   for(i=0;i<42;i++) {
	 ...
	 if(verifyDate<'2006-04-01')
	 {
		 cObj.href = "http://www.people.com.cn/GB/paper464/review/200903.html";
	 }
	 else if(verifyDate>="2006-04-01"&&verifyDate<"2008-06-12"){
		cObj.href = "http://paper.people.com.cn/rmrb/html/" + SY + "-" + appendZero(SM+1) +"/" +testDate + "/node_17.htm";
	 }
	 else
	 {
		 if(oPeriod)
...
function lYearDays(y) {
   var i, sum = 348;
   for(i=0x8000; i>0x8; i>>=1) sum += (lunarInfo[y-startYear] & i)? 1: 0;
   return(sum+leapDays(y));
}

//====================================== 传回农历 y年闰月的天数
function leapDays(y) {
   if(leapMonth(y))  return((lunarInfo[y-startYear] & 0x10000)? 30: 29);
   else return(0);
}

//====================================== 传回农历 y年闰哪个月 1-12 , 没闰传回 0
function leapMonth(y) {
   return(lunarInfo[y-startYear] & 0xf);
}

//====================================== 传回农历 y年m月的总天数
function monthDays(y,m) {
   return( (lunarInfo[y-startYear] & (0x10000>>m))? 30: 29 );
}
...
function Lunar(objDate) {
   ...
   var i, leap=0, temp=0;
   var offset   = (Date.UTC(objDate.getFullYear(),objDate.getMonth(),objDate.getDate()) - Date.UTC(startYear,0,31))/86400000;

for(i=startYear; i<nowYear && offset>0; i++) { temp=lYearDays(i); offset-=temp; }
...
//===== 某年的第n个节气为几日(从0小寒起算)
function sTerm(y,n) {
   var offDate = new Date( ( 31556925974.7*(y-startYear) + sTermInfo[n]*60000  ) + Date.UTC(startYear,0,6,2,5) );
...
//alert("calendar end 1");
   ...
   ////////年柱 2009年春分后为庚子年(60进制36)
   if(m<2) cY=cyclical(y-startYear+36-1);
   else cY=cyclical(y-startYear+22);
   var term2=sTerm(y,2); //立春日期

   ////////月柱 1900年1月小寒以前为 丙子月(60进制12)
   var firstNode = sTerm(y,m*2) //传回当月「节」为几日开始
   cM = cyclical((y-startYear)*12+m+12);
...
//alert("calendar end 2");
   for(var i=0;i<this.length;i++) {
      ...
      //依节气调整二月分的年柱, 以春分为界
      if(m==1 && (i+1)==term2) cY=cyclical(y-startYear+36);
      //依节气月柱, 以「节」为界
      if((i+1)==firstNode) cM = cyclical((y-startYear)*12+m+13);
      //日柱
...
function pushBtm(K) {
   switch (K){
      ...
      case 'MD' :
         ...
         break;
      default :
         CLD.SY.selectedIndex=tY-startYear;
...
function initialize() {
   ...
   var re = /\/(\d{4})-(\d{2})\/(\d{2})\/nbs.(\S+).htm/i;
   if(curUrl.match(re))
   {
   	tY = RegExp.$1 - 0;
   	tM = RegExp.$2 - 1;
...
if(document.getElementById("mobilepaper") !=null){
	document.getElementById("mobilepaper").style.display="block";
	document.getElementById("mobilepaper").style.height="30px";
	document.getElementById("mobilepaper").innerHTML='<a href="http://data.people.com.cn/rmrb/index.html" class="ared" style="font-size:16px" target="_blank"><span style="font-weight:bold;">人民日报图文数据库（1946-2017）</span></a><br><div style="height:1px;background-color:#99CCFF;margin-left:5px;line-height:1px;clear:both;display:block;overflow:hidden"></div>';
}
if(document.getElementById("xy20120611") !=null){
	document.getElementById("xy20120611").style.display="";
	document.getElementById("xy20120611").target="_blank";
}
//-->
//tongji
document.write("<div style='display:none'>");
var _bdhmProtocol = (("https:" == document.location.protocol) ? " https://" : " http://");
document.write(unescape("%3Cscript src='" + _bdhmProtocol + "hm.baidu.com/h.js%3F634d72f7004dc0d4fb6aff00de988bf1' type='text/javascript'%3E%3C/script%3E"));
document.write("</div>");
function openCheckBaiduImg(){
//关闭统计图片
...
var b=a[a.length-1];
  ...
  if(window.attachEvent !=null) window.attachEvent("onload",openCheckBaiduImg);
 else window.addEventListener("load",openCheckBaiduImg,false);
